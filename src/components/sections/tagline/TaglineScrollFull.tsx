...
// sm+ (768px+): full GSAP scroll-scrub with video seek, split words, zoom, peel.
// Identical to the original TaglineScrollSection — all breakpoint logic is internal.
...
type ViewportMetrics = { pillW: number; pillH: number; shellRadius: number; innerRadius: number; videoRadius: number; shellPad: number }
type SplitMetrics = { stageAFontSize: string; stageBFontSize: string; leftTargetX: number; rightTargetX: number }
type CachedLayout = { vw: number; vh: number; isDesktop: boolean; peelRadius: number; retreatX: number; connectorMaxW: number }

function isDesktopSplit(): boolean { return window.matchMedia('(min-width: 640px)').matches }

function postSplitProgress(p: number): number { return clamp(mapRange(p, SPLIT_END, 1, 0, 1), 0, 1) }

function splitEase(t: number): number { return gsap.parseEase('power2.inOut')(clamp(t, 0, 1)) }

function getViewportMetrics(vw: number): ViewportMetrics {
    const isMobile = vw < 1024
    const isNarrow = vw < 640
    const inset = isNarrow ? PILL_INSET_NARROW : PILL_INSET_TABLET
    if (!isMobile) return { pillW: PILL_W_DESKTOP, pillH: PILL_H_DESKTOP, shellRadius: SHELL_RADIUS, innerRadius: INNER_RADIUS, videoRadius: VIDEO_RADIUS, shellPad: SHELL_PAD }
    ...
    return { pillW, pillH, shellRadius: isNarrow ? 20 : SHELL_RADIUS, innerRadius: isNarrow ? 16 : INNER_RADIUS, videoRadius: isNarrow ? 16 : VIDEO_RADIUS, shellPad: isNarrow ? 6 : SHELL_PAD }
}
...
function buildCachedLayout(): CachedLayout {
    ...
    const isMobile = vw < 1024
    return { vw, vh, isDesktop: vw >= 640, peelRadius: isMobile ? PEEL_RADIUS_MOBILE : PEEL_RADIUS_DESKTOP, retreatX: vw < 1024 ? 52 : vw < 1280 ? 80 : 112, connectorMaxW: vw >= 1280 ? 140 : vw >= 1024 ? 96 : vw >= 640 ? 36 : 0 }
}

function bottomPeelRadius(px: number): string { return `0px 0px ${px}px ${px}px` }

function setConnectorLineWidth(el: HTMLElement | null, px: number): void { if (el) el.style.setProperty('--tagline-connector-w', `${px}px`) }
...
function canSeekVideo(video: HTMLVideoElement): boolean { return video.readyState >= 2 && Number.isFinite(video.duration) && video.duration > 0 }
...
function measureSplitMetrics(textLayer: HTMLElement, wordLeft: HTMLElement, wordRight: HTMLElement, leftConnector: HTMLElement, rightConnector: HTMLElement, vw: number): SplitMetrics {
    ...
    textLayer.classList.remove(STAGE_B_MEASURE_CLASS)
    gsap.set([wordLeft, wordRight], { clearProps: 'fontSize' })
    return { stageAFontSize, stageBFontSize, leftTargetX: -(leftRect.left - edgePad), rightTargetX: vw - rightRect.right - edgePad }
}
...
export function TaglineScrollFull() {
    ...
    const mobileTaglinesRef = useRef<HTMLDivElement>(null)
    ...
    const splitMetricsRef = useRef<SplitMetrics>({ stageAFontSize: '32px', stageBFontSize: '24px', leftTargetX: -200, rightTargetX: 200 })
    const layoutRef = useRef<CachedLayout>({ vw: 1280, vh: 800, isDesktop: true, peelRadius: PEEL_RADIUS_DESKTOP, retreatX: 112, connectorMaxW: 60 })
    ...
    const recomputeLayout = useCallback(() => {
        ...
        if (textLayer && wordLeft && wordRight && left && right && layout.isDesktop) {
            splitMetricsRef.current = measureSplitMetrics(textLayer, wordLeft, wordRight, left, right, layout.vw)
        }
    }, [])

    const revealStickyScene = useCallback(() => { stickyRef.current?.classList.remove(SCENE_PENDING_CLASS) }, [])
    ...
    useLayoutEffect(() => {
        ...
        if (!prefersReducedMotion) {
            ...
            const { isDesktop } = layoutRef.current
            if (stageRef.current) gsap.set(stageRef.current, { width: pillW, height: pillH, borderRadius: 0, autoAlpha: isDesktop ? 0 : 1, force3D: true })
            ...
            if (isDesktop) {
                for (const el of [leftRef.current, rightRef.current]) if (el) gsap.set(el, { x: 0, y: 0, zIndex: 30, force3D: true })
                for (const el of [wordLeftRef.current, wordRightRef.current]) if (el) gsap.set(el, { clearProps: 'fontSize' })
            ...
            if (mobileTaglinesRef.current) gsap.set(mobileTaglinesRef.current, { x: 0, y: 0, zIndex: 30, force3D: true })
    ...
    const applyProgress = useCallback((p: number) => {
        ...
        const { vw, vh, isDesktop, peelRadius, retreatX, connectorMaxW } = layoutRef.current
        ...
        const wordLeft = wordLeftRef.current
        const wordRight = wordRightRef.current
        const mobileTaglines = mobileTaglinesRef.current
        ...
        const animP = isDesktop ? postSplitProgress(p) : p

        let splitEased = 1, baseLeftX = 0, baseRightX = 0, pillAlpha = 1
        if (isDesktop) {
            if (p >= SPLIT_END) {
                splitEased = 1; baseLeftX = split.leftTargetX; baseRightX = split.rightTargetX; pillAlpha = 1
                if (wordLeft && wordRight) gsap.set([wordLeft, wordRight], { fontSize: split.stageBFontSize })
            } else if (p <= SPLIT_HOLD_END) {
                splitEased = 0; baseLeftX = 0; baseRightX = 0; pillAlpha = 0
                ...
                if (wordLeft && wordRight) gsap.set([wordLeft, wordRight], { fontSize: lerpFontSize(split.stageAFontSize, split.stageBFontSize, splitEased) })
            }
        ...
        if (mobileTaglines) gsap.set(mobileTaglines, { opacity: taglineOpacity, y: -lerp(0, 44, taglineRetreatT), zIndex: taglineZ, force3D: true })
        ...
        if (video && videoReadyRef.current && canSeekVideo(video)) {
            ...
            if (Math.abs(targetTime - lastVideoTimeRef.current) > 0.001) {
    ...
    const applyReducedMotionState = useCallback(() => {
        recomputeLayout()
        const split = splitMetricsRef.current
        const { isDesktop, connectorMaxW } = layoutRef.current
        ...
        if (isDesktop) {
            ...
            if (wordLeftRef.current && wordRightRef.current) gsap.set([wordLeftRef.current, wordRightRef.current], { fontSize: split.stageBFontSize })
            setConnectorLineWidth(leftRef.current, connectorMaxW)
            setConnectorLineWidth(rightRef.current, connectorMaxW)
        }
        if (mobileTaglinesRef.current) gsap.set(mobileTaglinesRef.current, { opacity: 1 })
    ...
    useEffect(() => {
        ...
        const primeVideo = () => {
            ...
            const animP = layoutRef.current.isDesktop ? postSplitProgress(progress) : progress
            ...
            if (!seekVideoTo(video, targetTime)) { video.removeEventListener('seeked', finishPrime); videoPrimedRef.current = false; return }
            if (Math.abs(video.currentTime - targetTime) < 0.001) { video.removeEventListener('seeked', finishPrime); finishPrime() }
        }
    ...
    useGSAP(
        () => {
            const container = containerRef.current
            if (!container) return
            const onResize = () => { recomputeLayout(); ScrollTrigger.refresh(); if (prefersReducedMotion) applyReducedMotionState(); else syncProgressRef.current(pendingProgressRef.current) }
            window.addEventListener('resize', onResize)
            if (prefersReducedMotion) { applyReducedMotionState(); revealStickyScene(); return () => window.removeEventListener('resize', onResize) }
            const st = ScrollTrigger.create({
                ...
                onUpdate(self) {
                    ...
                    requestAnimationFrame(() => { syncProgressRef.current(pendingProgressRef.current); tickingRef.current = false })
                },
            })
    ...
    const trackHeight = prefersReducedMotion ? 'h-svh' : 'h-[360vh] lg:h-[580vh]'
    ...
    return (
        ...
        >
            <div ref={stickyRef} data-tagline-sticky className={cn('sticky top-0 h-svh w-full overflow-hidden will-change-transform', !prefersReducedMotion && SCENE_PENDING_CLASS)}>
                <div ref={ambientRef} data-tagline-ambient aria-hidden className="pointer-events-none absolute inset-0 z-1 opacity-0"
                    style={{ background: 'radial-gradient(ellipse 70% 55% at 50% 45%, rgba(202,139,55,0.22) 0%, transparent 70%)' }} />

                <div ref={windowRef} data-tagline-window className="absolute inset-0 z-2 flex items-center justify-center overflow-hidden">
                    <div ref={stageRef} data-tagline-stage className="relative max-h-full max-w-full shrink-0 overflow-hidden will-change-[width,height]">
                        <div ref={shellRef} data-tagline-shell className="box-border h-full w-full overflow-hidden rounded-[24px] border border-black/8 p-2">
                            <div ref={innerRef} data-tagline-inner className={cn(faqAccordionInnerClassName(), 'relative h-full min-h-0! w-full gap-0! overflow-hidden px-0! py-0!', 'shadow-[0_40px_40px_-3.75px_rgba(0,0,0,0.02),0_20px_20px_-3px_rgba(0,0,0,0.03),0_11px_11px_-2.5px_rgba(0,0,0,0.04)]')}>
                                <video ref={videoRef} src="/images/tagline-strip.mp4" muted playsInline preload="auto" disablePictureInPicture aria-hidden
                                    className={cn('relative z-1 h-full w-full object-cover object-center opacity-0', 'bg-brand-surface transform-[translateZ(0)] backface-hidden')} />
                            </div>
                        </div>
                        <div ref={gradientRef} data-tagline-gradient aria-hidden
                    ...
                    <div ref={copyRef} data-tagline-copy
                        className={cn(
                        ...
                        <p className={cn('min-w-0 flex-1 text-left [font-family:var(--font-halant)] font-normal tracking-[-0.04em]', 'text-[15px] leading-[21px] text-white', 'sm:text-[17px] sm:leading-[23px]', 'md:text-brand-charcoal md:text-[22px] md:leading-[28px]', 'lg:text-[25px] lg:leading-[30px]')}>
                            {OVERLAY_COPY}
                        </p>
                    </div>
                </div>

                <div ref={mobileTaglinesRef} data-tagline-mobile
                    className={cn('pointer-events-none absolute right-0 left-0 z-30 flex flex-col items-center gap-1 px-4 text-center sm:hidden', 'top-[calc(50%-12rem)] max-[380px]:top-[calc(50%-10.75rem)]', 'will-change-[transform,opacity]')}>
                    <span className="text-brand-charcoal [font-family:var(--font-halant)] text-[clamp(1.25rem,5.5vw,1.375rem)] leading-tight font-normal tracking-[-0.04em]">Less chaos</span>
                    <span className="text-brand-charcoal [font-family:var(--font-halant)] text-[clamp(1.25rem,5.5vw,1.375rem)] leading-tight font-normal tracking-[-0.04em]">More Clarity</span>
                </div>

                <div ref={textLayerRef} data-tagline-text-layer
                    className={cn('pointer-events-none absolute inset-0 z-30 hidden items-center justify-center sm:flex', 'text-brand-charcoal [font-family:var(--font-halant)] font-normal tracking-[-0.04em]', 'text-[clamp(1.75rem,5vw,2.75rem)] leading-tight', 'lg:text-[clamp(3.75rem,5vw,5rem)] lg:leading-[1.1]', 'xl:text-[clamp(5rem,5.5vw,7rem)] xl:leading-[1.05]', 'overflow-visible')}>
                    <div className="-gap-x-[0.12em] flex items-center justify-center">
                        <div ref={leftRef} data-tagline-left className="tagline-connector tagline-connector-left flex shrink-0 items-center will-change-transform">
                            <span ref={wordLeftRef} data-word-left className="inline-block shrink-0 will-change-[transform,font-size]">Less chaos</span>
                        </div>
                        <div ref={rightRef} data-tagline-right className="tagline-connector tagline-connector-right flex shrink-0 items-center will-change-transform">
                            <span ref={wordRightRef} data-word-right className="inline-block shrink-0 will-change-[transform,font-size]">More Clarity</span>
                        </div>
                    </div>
                </div>
            </div>
