...
function styleHandoffImageInSlot(card: HTMLDivElement) {
    ...
    if (inner) inner.classList.add('relative', 'h-full', 'w-full')
    const img = card.querySelector<HTMLImageElement>('img')
    if (img) img.classList.add('object-cover', 'object-[center_35%]', 'h-full', 'w-full')
}
...
export function AboutVisionHandoffProvider({ children }: { children: ReactNode }) {
    ...
    const registerHandoffCard = useCallback((el: HTMLDivElement | null) => {
        if (!el) { if (handoffCardRef.current === el) handoffCardRef.current = null; return }
    ...
    const registerAboutPinScrollTrigger = useCallback((st: ScrollTrigger | null) => {
        ...
        if (st && aboutPinSuppressedRef.current) st.disable(false, false)
    }, [])
    ...
    const setAboutPinHandoffSuppressed = useCallback((suppressed: boolean) => {
        ...
        if (!st) return
        suppressed ? st.disable(false, false) : st.enable(false, false)
    }, [])
    ...
    const clearHandoffStartRect = useCallback(() => { handoffStartRectRef.current = null }, [])
