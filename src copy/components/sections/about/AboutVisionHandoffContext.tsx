...
/** Debounced global refresh after layout / reparent settles. */
...
export function AboutVisionHandoffProvider({ children }: { children: ReactNode }) {
  ...
  const detachHandoffFromVisionSlotForMorph = useCallback((): boolean => {
    ...
    if (handoffOriginClassNameRef.current) {
      card.className = handoffOriginClassNameRef.current
    }

    handoffInVisionSlotRef.current = false
    slot?.removeAttribute(VISION_HANDOFF_COMPLETE_ATTR)

    applyFixedHandoffRect(card, {
      left: rect.left,
      top: rect.top,
      width: rect.width,
      height: rect.height,
...
export function useAboutVisionHandoff() {
  ...
  if (!ctx) {
    throw new Error('useAboutVisionHandoff must be used within AboutVisionHandoffProvider')
  }
  return ctx
}
