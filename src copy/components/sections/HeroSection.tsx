...
const heroTierSnapshot = () =>
  typeof window !== 'undefined' ? getHeroTier() : 'lg'
...
const heroWordMaskClass =
  'relative -m-[0.15em] inline-block overflow-hidden p-[0.15em] align-top'
...
function HeroBottomCloudRail({
  ...
  return (
    <m.div
      ...
      className={cn(
        'pointer-events-none absolute bottom-[-32%] left-1/2 h-[44%] w-[170%] max-w-none -translate-x-1/2 translate-y-2',
        'sm:translate-y-3',
        'lg:bottom-[14%] lg:h-[52%] lg:w-[185%] lg:translate-y-4',
        gpuClass,
        className,
      )}
    >
      <div className="relative z-0 flex h-full w-full">
        <m.div
          style={leftX != null ? { x: leftX } : undefined}
          className={cn('relative h-full min-h-0 min-w-0 flex-1', gpuClass)}
        >
          <Image
        ...
        <m.div
          style={rightX != null ? { x: rightX } : undefined}
          className={cn('relative h-full min-h-0 min-w-0 flex-1', gpuClass)}
        >
          <Image
            src="/images/hero-cloud.png"
      ...
      <div className="pointer-events-none absolute inset-0 z-10 flex items-end justify-center">
        <m.div
          ...
          className={cn(
            'relative h-[88%] w-[42%] max-w-2xl min-w-[180px]',
            'sm:h-[90%] sm:w-[38%]',
            'lg:h-[92%] lg:w-[32%]',
            gpuClass,
          )}
...
export function HeroSection() {
  ...
  const textScrollSpring = useSpring(scrollYProgress, {
    stiffness: 260,
    damping: 36,
    mass: 0.2,
  })
  ...
  const helipadOpacity = useTransform(smoothProgress, [0, 0.35, 0.55], [0.9, 0.9, 0])
  ...
  const textOpacityLg = useTransform(
    textScrollSpring,
    [0, 0.32, 0.5, 0.66, 0.8],
    [1, 0.96, 0.5, 0.12, 0],
  )
  const heroTitleColorLg = useTransform(
    textScrollSpring,
    [0, 0.32, 0.5, 0.66],
    ['#1C1C1C', '#353533', '#7A7876', '#B5B3B1'],
  )
  const heroBodyColorLg = useTransform(
    textScrollSpring,
    [0, 0.32, 0.5, 0.66],
    ['#3F3F3E', '#555350', '#8E8C89', '#B9B7B4'],
  )
  const bookFlightOpacityLg = useTransform(
    textScrollSpring,
    [0, 0.22, 0.38, 0.52],
    [1, 0.78, 0.32, 0],
  )
  const bookFlightPointerEventsLg = useTransform(
    bookFlightOpacityLg,
    (v) => (v < 0.05 ? 'none' : 'auto'),
  )
  ...
  const cloudLeftXLg = useTransform(
    smoothProgress,
    [0, 0.03, 0.14, 0.9],
    ['0%', '0%', '-10%', '-38%'],
  )
  const cloudRightXLg = useTransform(
    smoothProgress,
    [0, 0.03, 0.14, 0.9],
    ['0%', '0%', '10%', '38%'],
  )
  const cloudYLg = useTransform(
    smoothProgress,
    [0, 0.03, 0.2, 0.74],
    ['0%', '0%', '6%', '22%'],
  )
  const cloudOpacityLg = useTransform(
    smoothProgress,
    [0, 0.03, 0.52, 0.9],
    [1, 1, 1, 0],
  )
  ...
  return (
    <>
      {isLg && (
        <m.div
          className={cn(
            'z-hero-helicopter pointer-events-none fixed inset-0',
            isBookingModalOpen && 'invisible',
          )}
        >
          <div className={cn('relative mx-auto h-full', HERO_MAIN_COLUMN_LAYOUT)}>
            {!RM && (
              ...
              >
                <AboutHelicopterRings className="mx-auto" />
              </m.div>
            )}
              ...
              >
                <div className="flex w-full max-w-full flex-col items-center justify-center lg:-translate-y-[min(5vh,3.5rem)] xl:-translate-y-[min(15vh,15rem)] 2xl:-translate-y-[min(15vh,15rem)]">
                  <m.div
                    ...
                    className="relative flex w-[min(92vw,42rem)] max-w-5xl flex-col items-center justify-end gap-0 opacity-90 lg:w-[88%] lg:max-w-7xl xl:w-[min(98vw,82rem)] 2xl:w-[min(98vw,80rem)]"
                  >
                    <m.div
                      animate={RM ? false : { y: [0, -10, 0] }}
                      transition={{ duration: 6, ease: easings.inOut, repeat: Infinity }}
                      className="relative z-10 aspect-4/3 w-full shrink-0 lg:max-h-[min(70vh,680px)] lg:w-[132%] lg:-translate-y-22 xl:max-h-[min(68vh,680px)] xl:w-[112%] xl:-translate-y-15 2xl:max-h-[min(72vh,760px)] 2xl:w-[118%] 2xl:-translate-y-24"
                    >
                      <Image
                    ...
                    <m.div
                      aria-hidden
                      style={{ opacity: RM ? 0.9 : helipadOpacity }}
                      className="absolute bottom-[28%] left-1/2 z-0 w-[50%] -translate-x-1/2"
                    >
                      <Image
              ...
              <m.div
                style={
                  RM
                    ? undefined
                    : {
                      opacity: bookFlightOpacityLg,
                      pointerEvents: bookFlightPointerEventsLg,
                    }
                }
      ...
      <div className="relative">
        <section ref={containerRef} className="bg-brand-surface relative isolate overflow-hidden">
          <div className={cn('relative z-10 mt-3 sm:mt-4', HERO_MAIN_COLUMN_LAYOUT)}>
            <div
              className={cn(
                'border-brand-white relative mx-auto w-full overflow-hidden',
                ...
                'min-h-[min(91svh,680px)]',
                'sm:min-h-[min(90svh,700px)]',
                'md:min-h-[min(90svh,780px)]',
                'lg:aspect-1408/1114 lg:max-h-[920px] lg:min-h-[min(70svh,540px)]',
            ...
            >
              <div className="pointer-events-none absolute inset-0 isolate">
                ...
                >
                  <div className="relative size-[min(80vw,420px)] shrink-0 sm:size-[min(70vw,480px)] md:size-[min(62vw,520px)] lg:size-[min(52vw,640px)] xl:size-[min(48vw,720px)]">
                    <Image
                      ...
                      alt=""
                      fill
                      className="object-contain opacity-30 blur-[48px] lg:opacity-40 lg:blur-[56px]"
                    />
                  </div>
                </m.div>
                ...
                <div
                  aria-hidden
                  className="pointer-events-none absolute inset-x-0 bottom-0 z-30 h-[30%] bg-linear-to-t from-white via-white/85 to-transparent sm:h-[32%] md:h-[34%] lg:h-[36%] lg:bottom-[7%] xl:bottom-[9%] xl:h-[34%] 2xl:bottom-[10%] 2xl:h-[32%]"
                />
              </div>

              <HeroBottomCloudRail
                className="z-[35] translate-y-1 md:translate-y-2"
              ...
              <div className="relative z-40 flex min-h-0 flex-col p-4 pb-7 sm:p-5 sm:pb-8 md:p-6 md:pb-10">
                <m.div
                  style={
                    RM || !isLg
                      ? undefined
                      : { y: textYLg, opacity: textOpacityLg }
                  }
                  variants={staggerContainer(0.1, 0.06)}
                  initial="hidden"
                  animate="visible"
                  className={cn(
                    'relative z-30 mx-auto flex w-full max-w-7xl flex-col',
                ...
                >
                  <div
                    className={cn(
                      ...
                      'xl:max-w-2xl xl:gap-3 xl:pt-14',
                      '2xl:max-w-136 2xl:pt-16',
                    )}
                  >
                    <m.div
                      variants={fadeInUp}
                      className="flex flex-row items-center gap-2 sm:gap-2.5 lg:gap-4 xl:gap-5"
                    >
                      <div className="relative flex size-11 shrink-0 rotate-180 items-center justify-center sm:size-12 lg:size-18 xl:size-20">
                        <Image
                          ...
                          width={80}
                          height={80}
                          className="size-11 rotate-180 object-contain sm:size-12 lg:size-18 xl:size-20"
                        />
                      </div>
                      <div
                        aria-hidden
                        className="h-[3px] w-[min(11rem,calc(100%-3rem))] shrink-0 self-center rounded-[48px] sm:w-[min(12rem,calc(100%-3.5rem))] lg:w-[min(16rem,calc(100%-5.5rem))] xl:w-[min(18rem,calc(100%-6rem))]"
                        style={{
                          background:
                  ...
                  >
                    ...
                    <m.p
                      ...
                      className={cn(
                        ...
                        'md:max-w-[26rem] md:text-[15px]',
                        'lg:ml-auto lg:max-w-[428px]',
                        'xl:max-w-md xl:text-[17px] xl:leading-[24px]',
          ...
          <div
            ...
            className="bg-hero-bottom-fade pointer-events-none absolute inset-x-0 bottom-0 z-75 h-[10%] lg:h-[40%] xl:h-[50%]"
          />
        </section>

        <AboutIntroSection
          fixedHelicopterOpacityRef={fixedHelicopterOpacityRef}
          fixedRingsRef={fixedRingsRef}
        />
      </div>
    </>
  )
