...
const FAQ_CARD_SHADOW =
  '0 40px 40px -3.75px rgba(0,0,0,0.02),0 20px 20px -3px rgba(0,0,0,0.03),0 11px 11px -2.5px rgba(0,0,0,0.04)'
...
type SplitMetrics = {
  stageAFontSize: string
  stageBFontSize: string
  leftTargetX: number
  rightTargetX: number
}
...
function getViewportMetrics(vw: number): ViewportMetrics {
  ...
  if (!isMobile) {
    return {
      pillW: PILL_W_DESKTOP,
      pillH: PILL_H_DESKTOP,
      shellRadius: SHELL_RADIUS,
      innerRadius: INNER_RADIUS,
      videoRadius: VIDEO_RADIUS,
      shellPad: SHELL_PAD,
    }
  }
...
function buildCachedLayout(): CachedLayout {
  ...
  return {
    ...
    vh,
    isDesktop: vw >= 640,
    peelRadius: isMobile ? PEEL_RADIUS_MOBILE : PEEL_RADIUS_DESKTOP,
    retreatX: vw < 1024 ? 52 : vw < 1280 ? 80 : 112,
    connectorMaxW: vw >= 1280 ? 140 : vw >= 1024 ? 96 : vw >= 640 ? 36 : 0,
  }
}

function bottomPeelRadius(px: number): string {
  return `0px 0px ${px}px ${px}px`
}
...
export function TaglineScrollSection() {
  ...
  const layoutRef = useRef<CachedLayout>({
    ...
    vh: 800,
    isDesktop: true,
    peelRadius: PEEL_RADIUS_DESKTOP,
    retreatX: 112,
    connectorMaxW: 60,
  })
  ...
  const recomputeLayout = useCallback(() => {
    ...
    if (textLayer && wordLeft && wordRight && left && right && layout.isDesktop) {
      splitMetricsRef.current = measureSplitMetrics(
        textLayer, wordLeft, wordRight, left, right, layout.vw
      )
    }
  }, [])
    ...
    if (!prefersReducedMotion) {
      ...
      if (stageRef.current) {
        gsap.set(stageRef.current, {
          width: pillW, height: pillH, borderRadius: 0,
          autoAlpha: isDesktop ? 0 : 1, force3D: true,
        })
      }
      if (windowRef.current) gsap.set(windowRef.current, { clearProps: 'transform' })
      if (shellRef.current) {
        gsap.set(shellRef.current, {
          borderRadius: shellRadius, padding: shellPad,
          borderWidth: SHELL_BORDER, force3D: true,
        })
      }
      if (innerRef.current) {
        gsap.set(innerRef.current, {
          ...
          boxShadow: FAQ_CARD_SHADOW, force3D: true,
        })
      }
      if (videoRef.current) {
        gsap.set(videoRef.current, { borderRadius: videoRadius, opacity: 0, force3D: true })
      ...
      if (mobileTaglinesRef.current) {
        gsap.set(mobileTaglinesRef.current, { x: 0, y: 0, zIndex: 30, force3D: true })
      }
  ...
  const applyProgress = useCallback((p: number) => {
    ...
    if (peelT > 0) {
      ...
      if (shell) {
        gsap.set(shell, {
          borderRadius: lerp(metrics.shellRadius, 0, zoomT),
          padding: lerp(metrics.shellPad, 0, zoomT),
          borderWidth: lerp(SHELL_BORDER, 0, Math.min(zoomT / SHELL_FADE_BY, 1)),
          force3D: true,
        })
      }
      if (inner) {
        gsap.set(inner, {
          borderRadius: lerp(metrics.innerRadius, 0, zoomT),
          borderColor: `rgba(255,255,255,${lerp(0.9, 0, shellFadeT)})`,
          boxShadow: shellFadeT >= 1 ? 'none' : FAQ_CARD_SHADOW,
          force3D: true,
        })
      }
    ...
    if (left) {
      gsap.set(left, {
        ...
        x: baseLeftX - lerp(0, retreatX, taglineRetreatT),
        zIndex: taglineZ,
        force3D: true,
      })
    }
    if (right) {
      gsap.set(right, {
        opacity: taglineOpacity,
        x: baseRightX + lerp(0, retreatX, taglineRetreatT),
        zIndex: taglineZ,
        force3D: true,
      })
    }
    if (mobileTaglines) {
      gsap.set(mobileTaglines, {
        opacity: taglineOpacity,
        y: -lerp(0, 44, taglineRetreatT),
        zIndex: taglineZ,
        force3D: true,
      })
    }
  ...
  const applyReducedMotionState = useCallback(() => {
    ...
    if (video) {
      gsap.set(video, { borderRadius: 0, opacity: 1 })
      video.pause()
      if (canSeekVideo(video)) seekVideoTo(video, video.duration * 0.5)
    }
    if (isDesktop) {
  ...
  useEffect(() => {
    ...
    const primeVideo = () => {
      ...
      const targetTime = prefersReducedMotion
        ? video.duration * 0.5
        : videoTimeForProgress(animP, video.duration)
  ...
  useGSAP(
    ...
    {
      scope: containerRef,
      dependencies: [prefersReducedMotion, applyReducedMotionState, revealStickyScene, recomputeLayout],
    }
  )
    ...
    >
      ...
      >
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse 70% 55% at 50% 45%, rgba(202, 139, 55, 0.22) 0%, transparent 70%)',
          }}
        />
        ...
        >
          ...
          >
            ...
            <div
              ...
              style={{
                background:
                  'linear-gradient(180deg, transparent 0%, rgba(9, 9, 11, 0.55) 55%, rgba(9, 9, 11, 0.88) 100%)',
              }}
            />
          </div>
          ...
          >
            <span
              aria-hidden
              className="md:bg-brand-charcoal w-0.5 shrink-0 self-stretch bg-white sm:w-1"
            />
            <p
              className={cn(
        ...
        <div
          ref={mobileTaglinesRef}
          data-tagline-mobile
          className={cn(
            'pointer-events-none absolute right-0 left-0 z-30 flex flex-col items-center gap-1 px-4 text-center sm:hidden',
            'top-[calc(50%-12rem)] max-[380px]:top-[calc(50%-10.75rem)]',
        ...
        <div
          ...
          className={cn(
            'pointer-events-none absolute inset-0 z-30 hidden items-center justify-center sm:flex',
            'text-brand-charcoal [font-family:var(--font-halant)] font-normal tracking-[-0.04em]',
            'text-[clamp(1.75rem,5vw,2.75rem)] leading-tight',
