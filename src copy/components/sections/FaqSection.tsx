      ...
      >
        ...
        <m.div layout className="relative z-10 flex w-full flex-col">
          <div className="flex min-h-[40px] items-center gap-3">
            <h3
              id={headingId}
              className="text-brand-charcoal min-w-0 flex-1 [font-family:var(--font-geist)] text-[18px] leading-[1.4] font-normal tracking-[-0.01em]"
            >
              {item.question}
            </h3>
            <span
              aria-hidden
              className="text-brand-charcoal flex h-10 w-10 shrink-0 items-center justify-center rounded-full"
            >
              {isOpen ? (
...
export function FaqSection() {
  ...
  /** Taller stack than Global Presence (7 cards) — stronger Y travel so the rail clears the viewport. */
        ...
        >
          <Container className="max-w-base flex min-h-0 w-full flex-1 flex-col gap-12 lg:flex-row lg:items-start lg:gap-[90px]">
            ...
            >
              ...
              <m.p
                variants={fadeInUp}
                className="text-brand-gray max-w-[408px] [font-family:var(--font-geist)] text-lg leading-normal"
              >
                From booking assistance to baggage details, our FAQs cover everything you need to
                know.
              </m.p>
            </m.aside>
