...
import { Fragment, type CSSProperties, type RefObject, useEffect, useRef } from 'react'
...
const DESKTOP_MOSAIC_SLOTS = DESKTOP_MOSAIC_COLUMNS.map((col) =>
  col.imageIndices.map((i) => REVEAL_IMAGES[i])
)
...
const CARD_SHARED_CLASS = cn(
  ...
  'lg:rounded-[16px] lg:p-2 xl:rounded-[22px]',
...
function AboutRevealSquareCard({
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 rounded-[14px] bg-linear-to-br from-transparent from-40% via-transparent via-55% to-[#090202]/8 sm:rounded-[24px] lg:rounded-[16px] xl:rounded-[22px]"
      />
      <div
  ...
  useGSAP(
    () => {
      ...
      if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        ...
        if (fixedHelicopterOpacityRef?.current)
          gsap.set(fixedHelicopterOpacityRef.current, { opacity: 1, clearProps: 'opacity' })
        ...
        scrollTrigger: {
          ...
          onEnterBack(self) {
            ...
            if (heroBottomFade && pinTimeline) {
              const { exitProgress } = readPinTimelineMeta(pinTimeline)
              gsap.set(heroBottomFade, {
                autoAlpha: aboutHeroFadeAlpha(self.progress, exitProgress),
              })
            }
            if (!handoffApiRef.current?.isBridgeHandoffActive?.()) scheduleScrollTriggerRefresh()
          },
      ...
      if (fixedHelicopterOpacityRef?.current)
        gsap.set(fixedHelicopterOpacityRef.current, { opacity: 1, scale: 1 })
      if (centerImage)
        gsap.set(centerImage, { autoAlpha: 1, scale: 1.8, y: 180, transformOrigin: 'center center' })

      if (ringsLayer) {
        tl.to(ringsLayer, { scale: 0.0, duration: 0.01, ease: 'power3.out' }, 0.12)
      }
      if (fixedHelicopterOpacityRef?.current) {
        tl.to(fixedHelicopterOpacityRef.current, { scale: 0.0, duration: 0.01, ease: 'power3.out' }, 0.12)
      }

      if (centerImage) {
        tl.fromTo(
          centerImage,
          { autoAlpha: 1, scale: 1.8, y: 180 },
          { autoAlpha: 1, scale: 1.0, y: 0, duration: 0.12, ease: 'none', immediateRender: false },
          0
        )
      ...
      tl.fromTo(
        badge,
        { autoAlpha: 0, y: 55 },
        { autoAlpha: 1, y: 0, duration: 0.15, ease: 'power2.out', immediateRender: false },
        0.10
      )
      ...
      mm.add('(min-width: 1024px)', () => {
        ...
        for (let i = 0; i < parallaxColsDesktop.length && i < COLUMN_PARALLAX_Y_RANGE_LG.length; i++) {
          ...
          tweens.push(
            tl.fromTo(col, { y: startY }, { y: endY, ease: 'none', duration: wordTimelineEnd, immediateRender: false }, 0)
          )
        }
      ...
      mm.add('(max-width: 1023px)', () => {
        ...
        for (let i = 0; i < parallaxColsMobile.length; i++) {
          ...
          const [startY, endY] =
            i === 0 ? MOBILE_LEFT_RAIL_PARALLAX_Y : ([startYMobile, -200] as const)
          gsap.set(col, { y: startY, force3D: true })
          tweens.push(
            tl.fromTo(col, { y: startY }, { y: endY, ease: 'none', duration: wordTimelineEnd, immediateRender: false }, 0)
          )
      ...
      const clearAboutGsapState = () => {
        ...
        if (centerImage)
          gsap.set(centerImage, { clearProps: 'opacity,transform,visibility,xPercent,yPercent,y,zIndex' })
        if (fixedHelicopterOpacityRef?.current)
          gsap.set(fixedHelicopterOpacityRef.current, { clearProps: 'opacity' })
        handoffPeers.forEach((el) => gsap.set(el, { clearProps: 'opacity,transform,visibility' }))
        if (handoffAnchor)
          gsap.set(handoffAnchor, { clearProps: 'opacity,visibility,transform,scale,zIndex,y' })
    ...
    {
      scope: pinRef,
      dependencies: [prefersReducedMotion, fixedHelicopterOpacityRef, fixedRingsRef],
    }
  )
        ...
        <div ref={pinRef} className="relative z-[55] w-full">
          <div data-about-stage className="relative min-h-svh w-full">
            ...
            >
              ...
              >
                <div
                  data-about-parallax-col-mobile
                  className="shrink-0 will-change-transform lg:hidden"
                >
                ...
                >
                  <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-[100]">
                    <div
                      data-about-center-image
                      className="pointer-events-auto relative z-50 will-change-transform"
                    >
                      <div className={cn(
                        'relative aspect-square w-(--about-card) shrink-0 overflow-visible',
                        'rounded-[18px] border-2 border-[#090202]/8 p-1.5',
                        'sm:rounded-[24px] sm:p-2',
                        'bg-about-reveal-frame-outer/8 shadow-[0_14px_44px_-18px_rgba(9,9,11,0.22)]',
                        'backdrop-blur-none md:backdrop-blur-md lg:backdrop-blur-xl',
                      )}>
                  ...
                  <div
                    data-about-badge
                    className="relative z-50 flex flex-col items-center gap-1.5 sm:gap-2"
                  >
                    ...
                    <Image
                      ...
                      height={12}
                      alt=""
                      className="h-auto w-[min(200px,calc(100vw-2.5rem))] max-w-full shrink-0 sm:w-[min(220px,calc(100vw-2rem))]"
                      aria-hidden
                    />
                  </div>
                  ...
                  <div data-about-copy-inner className="w-full">
                    <p className={cn(
                      ...
                      'sm:max-w-[min(34rem,calc(100vw-2rem))]',
                      'md:max-w-[min(40rem,calc(100vw-2.25rem))]',
